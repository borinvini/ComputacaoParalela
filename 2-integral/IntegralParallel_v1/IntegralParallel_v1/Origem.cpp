...
{
	int i, nthreads;
	double pi, sum[NUM_THREADS]; //array para armazenar todas as somas
...
#pragma omp parallel
	{
		...
		for (i = id, sum[id] = 0.0; i < num_steps; i = i + nthrds) //distribuição cíclica (distribuição round robin)
